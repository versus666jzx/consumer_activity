...
def show_data_samples(n_samples: int, data_list: list):
    ...
    Описание колонок:
        ...
        date - дата создания записи в бд
        timeThirty - получасовые интервалы времени
        revenue - выручка в данный момент времени
        point - точка продаж
        timeHour - часовые интервалы времени
    """)
    ...
    Описание колонок:
        ...
        Дата рейса - дата рейса	
        Рейс - номер рейса
        Вход в чистую зону - время входа в чистую зону
        Выход на рейс - время выхода на рейс
        Терминал - название терминала
        Авиакомпания - название авиакомпании
        Направление куда летит - направление полёта
    """)
    ...
    Описание колонок:
        ...
        AIRLINE - номер рейса
        REG - регистрационный номер самолёта
        INB - прилёт
        INB_FLIGHT - номер рейса прилёта
        INB_BEST_TIME - время прилёта
        INB_ROU - точка прилёта
        OUT	- вылет
        OUT_FLIGHT - номер рейса вылета
        OUT_BEST_TIME - время вылета
        OUT_ROU - точка вылета
...
def hypothesis_and_segmentation_block(all_data_list: list[pd.DataFrame, ...]):
    ...
    2. Для предсказания выручки каждой точки продаж преобразуем данные по выручке таким образом,
       чтобы данные по каждой точке продаж находились в отдельной колонке. В дальнейшем мы сможем
    ...
    st.write("""
    4. Также выдвинем гипотезу о том, что выручка зависит от погоды. Данные о погоде у нас уже находятся в таблице.
    """)
...
def fit_and_evaluate_model(data: pd.DataFrame, data_stock: pd.DataFrame, task_type: str = "CPU"):
    ...
    st.table(res)
    st.write("""
    Меньшие значения метрик mean_root_squared_error, mean_absolute_error и max_error - лучше.
    Большее значение метрики r2_score - лучше.
    """)
...
def practice_part():
    ...
    st.write("### Практическая часть")
    with st.form("practice"):
        answer_1 = st.radio(
            label="Какую задачу мы решали в данном уроке?",
            options=[
                "Задачу анализа потребительской активности в аэропорту",
                "Задачу объединения большого количества данных в одну таблицу",
                "Задачу разработки модели, для прогноза потребительской активности в аэропорту"
            ]
        )
        if answer_1 == "Задачу разработки модели, для прогноза потребительской активности в аэропорту":
            true_answers_count += 1
            st.success("Верно!")

        answer_2 = st.radio(
            label="Как можно улучшить прогноз модели, работая с открытыми данными?",
            options=[
                "Использовать стандартные значения гиперпараметров выбранной модели",
                "Дополнить имеющиеся данные, данными из открытых источников"
            ]
        )
        if answer_2 == "Дополнить имеющиеся данные, данными из открытых источников":
            true_answers_count += 1
            st.success("Верно!")

        answer_3 = st.radio(
            label="Данные из какого внешнего источника не использовались в уроке?",
            options=[
                "данные с сайта flightradar24",
                "данные о погоде",
                "данные о количестве мест в самолетах",
                "данные сайта svo.aero"
            ]
        )
        if answer_3 == "данные сайта svo.aero":
            true_answers_count += 1
            st.success("Верно!")

        answer_4 = st.radio(
            label="В данном уроке мы работали с кодами ИАТА. Что это?",
            options=[
                "Коды самолётов в международной системе учёта самолётов",
                "Геокод, обозначающий аэропорты и мегаполисы по всему миру",
                "Коды, обозначающие точки продаж в чистой зоне аэропорта"
            ]
        )
        if answer_4 == "Геокод, обозначающий аэропорты и мегаполисы по всему миру":
            true_answers_count += 1
            st.success("Верно!")

        answer_5 = st.radio(
            label="Какое количество гипотез мы выдвинули?",
            options=[
                "2",
                "3",
                "4",
        ...
        answer_6 = st.radio(
            label="Какая из гипотез не подтвердилась?",
            options=[
        ...
        if answer_6 == "Есть зависимость выручки от длины полета":
            true_answers_count += 1
            st.success("Верно!")

        answer_7 = st.radio(
            label="В какое время суток выручка точек продаж была минимальной?",
            options=[
                "Утро",
                "День",
                "Вечер",
        ...
        get_res = st.form_submit_button("Сдать зачёт")

        if get_res:
            if true_answers_count < 6:
                st.error("Зачет не сдан. Попробуйте еще раз!")
            else:
                st.success("Поздравляем! Вы успешно усвоили пройденный материал, нам приятно :)")
